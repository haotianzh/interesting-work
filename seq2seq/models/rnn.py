...
class RnnAutoencoder(object):
    def __init__(self, rnn_type, dim_emb, num_units, batch_size, vocab_size, seq_len, 
        ...
        self.zero_for_name = tf.constant(0.0)
    ...
    def build_model(self, inputs, infer):
        ...
        output, last_state = seq2seq.rnn_decoder(dec_in, self.initial_state, cell_dec, scope='decoder')
    ...
    def generate_original(self, sess, vocab, sym_x, sym_lx, sym_y, x_in, lx_in, beam_size, max_seq_len, idx2word = None, black_reply_dict = None):
        ...
        #print("state debugstring:", state)
        end = time.clock()
        run_total += end - start
        print("Run init cost : %f s" % (end - start))
        live_k, dead_k = 1, 0
        ...
        for i in range(max_seq_len):
            ...
            prob, state = sess.run([self.log_probs, self.last_state], {sym_y: yi, self.initial_state: state})
            #print("prob debugstring:", prob)
            end = time.clock()
            run_total += end - start
            #print("Run cost : %f s" % (end - start))
            start = time.clock()
            #cand_scores = hyp_scores[:, None] - np.log(prob)
            cand_scores = hyp_scores[:, None] - prob
            print("cand_scores debugstring:", cand_scores)
            end = time.clock()
            recalc_total += end - start
            print("Recalc score cost : %f s" % (end - start))
            cand_flat = cand_scores.flatten()
            start = time.clock()
            ranks_flat = cand_flat.argsort()[: (beam_size - dead_k)]
            end = time.clock()
            topn_total += end - start
            print("get top n cost : %f s" % (end - start))
        ...
        print("run total cost:", run_total*1000)
        print("recalc total cost:", recalc_total*1000)
        print("topn total cost:", topn_total*1000)
