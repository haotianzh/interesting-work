...
def load_args():
    ...
    parser.add_argument('--data_dir', type=str, default='../data/train_data_toy/', \
    ...
    parser.add_argument('--num_units', type=int, default=511, \
    ...
    parser.add_argument('--device', type=str, default='/gpu:7',
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,
            ...
            for e in range(args.num_epochs):
                ...
                for b in range(num_batches_train):
                    ...
                    if (current_step % args.save_every == 0) or (e == args.num_epochs - 1 and b == num_batches_train - 1):
                        ...
                        with g2.as_default():
                            ...
                            tf.import_graph_def(g1.as_graph_def(),
                                    input_map={name:consts[name] for name in consts.keys()})
                            tf.train.write_graph(g2.as_graph_def(),
                                    model_dir, 'rnn.pb.%s' % (current_step), False)
                            tf.train.write_graph(g2.as_graph_def(),
                                    model_dir, 'rnn.txt.%s' % (current_step))
                        sys.exit(1)
                    '''
                # valid
                outs = []
